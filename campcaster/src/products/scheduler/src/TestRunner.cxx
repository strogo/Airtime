/*------------------------------------------------------------------------------
    ...
    Location : $URL: svn://code.campware.org/campcaster/trunk/campcaster/src/products/scheduler/src/TestRunner.cxx $
 ...
 */
static const char   options[] = "c:ho:s:v";
...
/**
 *  The name of the scheduler config file to use.
 */
static Ptr<std::string>::Ref configFileName;
        ...
        char  * argv[] )                                throw ()
{
    ...
    // Configure the test methods via their shared base class
    try {
        LiveSupport::Scheduler::BaseTestMethod::configure(*configFileName);
...
processArguments(int argc, char *argv[])
{
    ...
    while ((i = getopt_long(argc, argv, options, longOptions, 0)) != -1) {
        switch (i) {
            case 'c':
                configFileName.reset(new std::string(optarg));
                break;
    ...
    if (!configFileName) {
        std::cerr << "mandatory option configuration file name not specified"
                  << std::endl;
        printUsage(argv[0], std::cout);
        return false;
    }
